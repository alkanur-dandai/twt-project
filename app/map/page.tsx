...
export default function MapPage() {
  ...
  // Force map resize on mount and orientation change for mobile viewport compatibility
  useEffect(() => {
  ...
  const hoveredFeature = useMemo(() => {
    ...
    return (
      allFeatures.find(
        (feature) => feature.properties?.adm4_en === hoveredName
      ) ?? null
    );
  }, [hoveredName]);
  ...
  // Consolidate features into a single collection to avoid WebGL context limits on mobile
  const consolidatedPolygons = useMemo(() => {
    const features = geoJsonLayers.flatMap((layer) =>
      layer.features
        .filter((feature: any) => Boolean(feature.properties?.adm4_en))
        .map((feature: any) => {
          const name = feature.properties?.adm4_en;
          const hasPopulationData = layer === barangays;
          const population = hasPopulationData ? getBarangayPopulation(name) : null;

          const fillColor =
            showPopulation && hasPopulationData && population !== null
              ? getPopulationColor(population)
              : "#fcfcfc00";

          const fillOpacity = showPopulation && hasPopulationData ? 0.45 : 0.05;
  ...
  return (
    <main className="relative h-dvh w-full overflow-hidden">
      ...
      >
        ...
        {/* Consolidated Single Layer for Polygons */}
        <MapGeoJSON
          data={consolidatedPolygons}
          interactive
          onHover={(e) => {
        ...
        {/* Hover Name Card - pointer-events-none prevents blocking interactions */}
        {hoveredFeature && (
          <div className="pointer-events-none absolute top-1/2 left-1/2 z-50 -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white px-4 py-2 shadow-lg">
            <p className="text-sm font-semibold text-gray-900">
              {hoveredFeature.properties?.adm4_en}
            </p>
          </div>
        ...
        {municipalityFeature && municipalityColor && (
          <MapGeoJSON
            data={municipalityFeature}
            interactive={false}
            fillPaint={{
